function [out, ratio] = ratioChan_LMv3(img)
...
ColormapScale  = 4;    % Sets the colormap max value; original value 8. Too high?

ColormapBins   = 20;   % This sets the number of bins in the colormap

MinPixelH5     = 300;
MaxPixelH5     = 2000;  % This is the maximum intensity of the pixels in image default 65000
MinPixelFn     = 200;
MaxPixelFn     = 2000;
%MinPixel and MaxPixel enable you to transfer the Black point and White
%points from volocity contrast enhancements
%%
%--------------------------------------------------------------------------
close all
...
%on confocal 640=ch1 560=ch2 488=ch3 405=ch4  in (img,ch)

img1 = mat2gray(imread(img,2),[MinPixelH5 MaxPixelH5]); %H5) channel - scale to pixel intensity
img2 = mat2gray(imread(img,1),[MinPixelFn MaxPixelFn]); %Fn) channel - scale to pixel intensity
...
if Thresh_combine == 0
    ...
    %mask1 = im2bw(img1,0); %This mask is constructed of the first channel
    mask1 = im2bw(img1,threshold1); %This mask is constructed of the first channel
    threshold2 = graythresh(img2) %This is the Otsu threshold for img2
    %mask2 = im2bw(img2,0); %This mask is constructed of the second channel
